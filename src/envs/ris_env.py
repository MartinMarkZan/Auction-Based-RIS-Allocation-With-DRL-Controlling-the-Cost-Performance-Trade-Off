...
from src.sim.geometry import gen_POS_cell_edge
...
class RISAuctionEnv(ParallelEnv):
    ...
    def __init__(self, cfg: Config, render_mode: str | None = None) -> None:
        ...
        self.util_alpha = self.cfg.util_alpha
    ...
    def observation_space(self, agent: str) -> spaces.Box:
        ...
        return spaces.Box(low=0.0, high=1.0, shape=(self.N_RIS + 2,))
    ...
    def reset(self, seed: int | None = None, options: dict | None = None
        ...
        self.current_value = {agent: 0 for agent in self.agents}
        obs_values = -1
        ...
        observations = {
            agent: np.append(np.append(obs_values[agent], self.price / self.max_budget[agent] * (1 - self.delta)), 1)
            for agent in self.agents
        }
    ...
    def step(self, actions: dict[str, np.ndarray]
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            if won_ris.any():
                ...
                self.current_value[agent], _, _, _ = est_values_pos(self.N_BS, self.N_RIS, self.N_UE, self.M_BS, self.M_RIS, self.M_UE, agent_idx, self.ps_lin, self.sigma_n2,
                                                        self.pow_ue_bs[:, :, no], self.pow_ris_bs[:, :, no], self.pow_ris_ue[:, :, no], self.util_alpha,
                                                        self.BS_UE_assoc[:, no], RIS_alloc, self.IBI[:, :, :, no], self.LOS_UE_BS[:, :, no], self.LOS_RIS_BS[:, :, no],
                                                        self.LOS_RIS_UE[:, :, no], self.K)
                self.current_value[agent] = (self.current_value[agent][0] / self.base_values[agent_idx] - 1)
            ...
            rewards[agent] = infos[agent]["R1"] - infos[agent]["R2"] - infos[agent]["R3"]
    ...
    def reset_geometry(self) -> dict[str, np.ndarray]:
        ...
        """
        # Generate positions.
        self.UE_pos, self.BS_pos, self.RIS_pos = gen_POS_cell_edge(self.N_OP, 
        ...
        self.base_values = np.zeros((len(self.agents),))

        # TODO: Generalize to multiple operators.
        no = 0
        for agent_idx in range(len(self.agents)):
            value, _, _, _ = est_values_pos(self.N_BS, self.N_RIS, self.N_UE, self.M_BS, self.M_RIS, self.M_UE, agent_idx, self.ps_lin, self.sigma_n2,
                                        self.pow_ue_bs[:, :, no], self.pow_ris_bs[:, :, no], self.pow_ris_ue[:, :, no], self.util_alpha,
                                        self.BS_UE_assoc[:, no], np.zeros((1, self.N_RIS)), self.IBI[:, :, :, no], self.LOS_UE_BS[:, :, no], self.LOS_RIS_BS[:, :, no],
                                        self.LOS_RIS_UE[:, :, no], self.K)
            self.base_values[agent_idx] = value
        
        # Force computation of initial values.
        initial_actions = {agent: np.ones(self.N_RIS, dtype=bool) for agent in self.agents}
        obs_values = self.update_value(initial_actions, force=True)

        return obs_values


    def update_value(self, actions: dict[str, np.ndarray], force: bool = False
        ) -> dict[str, np.ndarray]:
        """
        ...
        Args:
            actions: For each agent, a binary vector indicating a bid (1) or no 
                bid (0) for each RIS.
            force: If True, recompute valuations even if the agent’s RIS allocation
                has not changed since the previous step.
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            if not (force or changed) or (candidates.size == 0):
            ...
            # Evaluate.
            values, _, _, _ = est_values_pos(self.N_BS, self.N_RIS, self.N_UE, self.M_BS, self.M_RIS, self.M_UE, agent_idx, self.ps_lin, self.sigma_n2,
                            self.pow_ue_bs[:, :, no], self.pow_ris_bs[:, :, no], self.pow_ris_ue[:, :, no], self.util_alpha,
                            self.BS_UE_assoc[:, no], RIS_allocs, self.IBI[:, :, :, no], self.LOS_UE_BS[:, :, no], self.LOS_RIS_BS[:, :, no],
                            self.LOS_RIS_UE[:, :, no], self.K)

            # Normalize gains.
            gains = (values / self.base_values[agent_idx]) - 1.0
            gains -= self.current_value[agent]
            if np.max(np.abs(gains)) != 0:
                gains /= np.max(np.abs(gains))

            # Place into RIS vector.
            agent_values = np.zeros(self.N_RIS)
            agent_values[candidates] = gains
            agent_values[unavailable] = 0.0
